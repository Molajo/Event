...
Simple, uniform File and Directory Services API for PHP applications enabling interaction with multiple Event types
...
**Event** provides a common API to read, list, write, rename, delete, copy or move files and folders
...
to fileservices for application support services, like Event, Logging, Message, and Event.

## Basic Usage ##

Each **Event** command shares the same syntax and the same four parameters:

### Event Request ###

```php
    $adapter = new Molajo/Event/Adapter($action, $path, $filesystem_type, $options);
```
#### Parameters ####
...
Metadata is defined in the [Molajo\Document\Event\Properties](https://github.com/Molajo/Event/blob/master/src/Molajo/Event/Type/EventType.php) class. The Properties
class can be extended and customized, as needed, by Event.
...
```php
    $adapter = new \Molajo\Document\Event\Adapter('Read', 'location/of/file.txt');
    echo $adapter->fs->data;
```
...
```php
    ...
    $adapter = new \Molajo\Document\Event\Adapter('List', 'directory-name', $options);
...
```php
    ...
    $adapter      = new \Molajo\Document\Event\Adapter('Write', 'standard', $options, 'Log');
```
...
```php
    ...
    $adapter = new \Molajo\Document\Event\Adapter('Copy', 'name/of/source/file', $options);
```
...
```php
    ...
    $adapter = new \Molajo\Document\Event\Adapter('Move', 'current/folder/year2012', $options);
```
...
```php
    $adapter = new \Molajo\Document\Event\Adapter('Delete', 'name/of/source/folder');
```
...
```php
    $adapter = new \Molajo\Document\Event\Adapter('List', '/first/location');
    $data1   = $adapter->fs->data;

    $adapter = new \Molajo\Document\Event\Adapter('List', '/second/location');
...
```php
    ...
    $adapter = new \Molajo\Document\Event\File($options);
    $data    = $adapter->backup ();
```
...
This instructs PHP to use Composer’s autoloader for **Event** project dependencies.

#### Or, Install Manually

Download and extract **Event**.

Copy the Molajo folder (first subfolder of src) into your Vendor directory.

Register Molajo\Document\Event\ subfolder in your autoload process.
...
**Molajo Event** is licensed under the MIT License - see the `LICENSE` file for details
...
More Information
----------------
- [Usage](/Event/doc/usage.md)
- [Extend](/Event/doc/extend.md)
- [Specifications](/Event/doc/specifications.md)
